...
j += 1import face_recognition as fr
import os, glob, socket, time
from http.server import BaseHTTPRequestHandler, HTTPServer
import base64
import numpy as np
...
while i < fileCount:
	...
	i += 1
print(baseImages[0])
print(fr.face_encodings(baseImages[0]))
for img in baseImages:
	encodedArray.append((fr.face_encodings(img)[0]).astype(np.uint8))
